...
通过调用`SimpleEventPlugin.registerEvents`，进行事件的`onClick->click`,并保存在`topLevelEventsToReactNames`中，然后调用`registerTwoPhaseEvent`函数，注册两个阶段的事件，`冒泡`和`捕获`事件，将事件添加到`allNativeEvents`中
...
export function listenToAllSupportedEvents(rootContainerElement: EventTarget) {
  if (enableEagerRootListeners) {
    // 1. 节流优化, 保证全局注册只被调用一次，避免重复注册
    if ((rootContainerElement: any)[listeningMarker]) {
      return;
    }
    (rootContainerElement: any)[listeningMarker] = true;
    // 2. 遍历allNativeEvents 遍历allNativeEvents进行循环绑定
    allNativeEvents.forEach(domEventName => {
      // 不需要代理的事件
      if (!nonDelegatedEvents.has(domEventName)) {
        listenToNativeEvent(
...
[listenToNativeEvent](https://github.com/facebook/react/blob/v17.0.2/packages/react-dom/src/events/DOMPluginEventSystem.js#L351-L412)：将所有支持的事件在container上全都注册上


[addTrappedEventListener](https://github.com/facebook/react/blob/v17.0.2/packages/react-dom/src/events/DOMPluginEventSystem.js#L468-L560)：构建冒泡/捕获的事件监听
...
export function createEventListenerWrapperWithPriority(
  ...
  switch (eventPriority) {
    case DiscreteEvent: // 事件优先级最高
      listenerWrapper = dispatchDiscreteEvent;
      break;
    case UserBlockingEvent: // 事件优先级适中
      listenerWrapper = dispatchUserBlockingUpdate;
      break;
    case ContinuousEvent: // 事件优先级最低
    default:
...
1. `attemptToDispatchEvent`：把原生事件和Fiber树关联起来
2. `SimpleEventPlugin.extractEvents`：初始化时间系统的类
3. `processDispatchQueue`：执行派发
...
1. 收集所有`listener`回调
   ...
   - 具体逻辑在[accumulateSinglePhaseListeners](https://github.com/facebook/react/blob/v17.0.2/packages/react-dom/src/events/DOMPluginEventSystem.js#L712-L803):递归向上搜集listener
       ...
       while (instance !== null) {
         ...
         if (tag === HostComponent && stateNode !== null) {
           ...
           if (reactEventName !== null) {
             // 从DOM上获取合成事件 (如onClick , onClickCapture等)
...
function processDispatchQueueItemsInOrder(
  ...
  if (inCapturePhase) {
    ...
    for (let i = dispatchListeners.length - 1; i >= 0; i--) {
      // instance:Fiber节点  currentTarget:真实Dom listener:onClick之类的事件
